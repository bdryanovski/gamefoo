...
export class DialogRunner {
  ...
  private resolveTree(ref: string): DialogTreeData | null {
    ...
    if (Object.prototype.hasOwnProperty.call(trees, ref)) {
      return trees[ref]!;
    }
  ...
  get phase(): DialogPhase {
    if (this.revealed < this.segmentText.length) {
      return 'typing';
    }
  ...
  get choices(): DialogChoice[] {
    ...
    if (options.length === 0) {
      return [{ label: this.closeLabel, target: null }];
    }
  ...
  moveSelection(delta: number): void {
    if (!this.running || this.phase !== 'choosing') {
      return;
    }
