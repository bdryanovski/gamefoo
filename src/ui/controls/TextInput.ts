...
import type { UICharInputEvent, UIInputEvent, UIKeyEvent, UISize } from '../core/types';
...
export default class TextInput extends UIWidget {
  ...
  protected drawSelf(ctx: RenderContext): void {
    try {
      ...
      // Draw background
      ctx.fillRect(0, 0, this._width, this._height, theme.colors['input.background']);
      ctx.strokeRect(0, 0, this._width, this._height, theme.colors['input.border']);
      ...
      if (this._selectionStart >= 0) {
        const start = Math.min(this._selectionStart, this._cursorPosition);
        const end = Math.max(this._selectionStart, this._cursorPosition);
        const selStartX = textX + font.getTextWidth(this.getDisplayText().slice(0, start));
        const selWidth = font.getTextWidth(this.getDisplayText().slice(start, end));

        ctx.fillRect(selStartX, textY, selWidth, font.height, theme.colors['input.selection']);
      }
      ...
      if (displayText) {
        ...
        if (canvas) {
          ...
          ctx.drawText(this._placeholder, textX, textY, theme.colors['input.placeholder']);
        }
      }

      // Draw cursor (when focused and visible)
      if (this._cursorVisible) {
        const cursorX = textX + font.getTextWidth(displayText.slice(0, this._cursorPosition));
        ctx.fillRect(cursorX, textY, 1, font.height, theme.colors['input.caret']);
      }
    } catch {
      // Fallback rendering
