...
/**
 * The "bat" critter, bound to the `rat` object and driven by its exported
 * state machine (`Idle`/`Up`/`Down`/`Left`/`Right`). It runs its own simple
 * AI every frame:
 *
 * - **Flee** — when the player enters the bat's authored `vision` collider it
 ...
 * The bat can't see the game-owned player on its own, so the game feeds it the
 * player's box + the screen collision each frame via {@link bat.sense}; the
 * state machine is switched to match the heading so the animation follows.
 */
export class Bat extends ExploreAI {
  static override readonly type = 'bat';
}
